...
import torch
from monai.networks.blocks.regunet_block import (RegistrationDownSampleBlock,
...
__all__ = ["RegUNet", "RigidHead", "GlobalNetRigid", "LocalNet"]
...
class RegUNet(nn.Module):
    ...
    def __init__(
        ...
        self.num_channels = [
            self.num_channel_initial * (2**d) for d in range(self.depth + 1)
        ]
    ...
    def build_encode_layers(self):
        ...
        self.encode_convs = nn.ModuleList(
            [
                self.build_conv_block(
                    in_channels=(
                        self.in_channels if d == 0 else self.num_channels[d - 1]
                    ),
        ...
        self.encode_pools = nn.ModuleList(
            [
                self.build_down_sampling_block(channels=self.num_channels[d])
                for d in range(self.depth)
            ]
        )
    ...
    def build_down_sampling_block(self, channels: int):
        return RegistrationDownSampleBlock(
            spatial_dims=self.spatial_dims, channels=channels, pooling=self.pooling
        )
    ...
    def build_decode_layers(self):
        self.decode_deconvs = nn.ModuleList(
            [
                self.build_up_sampling_block(
                    in_channels=self.num_channels[d + 1],
                    out_channels=self.num_channels[d],
                )
        ...
        self.decode_convs = nn.ModuleList(
            [
                self.build_conv_block(
                    in_channels=(
                        2 * self.num_channels[d]
                        if self.concat_skip
                        else self.num_channels[d]
                    ),
                    out_channels=self.num_channels[d],
                    kernel_size=3,
                )
    ...
    def forward(self, x):
        ...
        for i, (decode_deconv, decode_conv) in enumerate(
            zip(self.decode_deconvs, self.decode_convs)
        ):
...
class RigidHead(nn.Module):
    def __init__(
        ...
        if spatial_dims == 2:
            ...
            out_features = 3
            out_init = torch.tensor([0, 0, 0], dtype=torch.float)
        elif spatial_dims == 3:
            ...
            raise ValueError(
                f"only support 2D/3D operation, got spatial_dims={spatial_dims}"
            )
    ...
    def rigid_transform(self, theta: torch.Tensor):
        ...
        if self.spatial_dims == 2:
            # Extract rotation angle and translations
            ...
            theta_new = torch.zeros((theta.shape[0], 2, 3), device=theta.device)
            theta_new[:, 0, 0] = cos_theta
            theta_new[:, 0, 1] = -sin_theta
            theta_new[:, 0, 2] = t_x
            theta_new[:, 1, 0] = sin_theta
            theta_new[:, 1, 1] = cos_theta
            theta_new[:, 1, 2] = t_y

            grid_warped = torch.einsum("qij,bpq->bpij", grid_padded, theta_new)
        elif self.spatial_dims == 3:

            theta_new = torch.zeros((theta.shape[0], 12), device=theta.device)

            theta_new[:, 0] = torch.cos(theta[:, 1]) * torch.cos(theta[:, 0])
            theta_new[:, 1] = torch.sin(theta[:, 2]) * torch.sin(
                theta[:, 1]
            ) * torch.cos(theta[:, 0]) - torch.cos(theta[:, 2]) * torch.sin(theta[:, 0])
            theta_new[:, 2] = torch.sin(theta[:, 2]) * torch.sin(
                theta[:, 0]
            ) + torch.cos(theta[:, 2]) * torch.sin(theta[:, 1]) * torch.cos(theta[:, 0])
            theta_new[:, 3] = theta[:, 3]
            theta_new[:, 4] = torch.cos(theta[:, 1]) * torch.sin(theta[:, 0])
            theta_new[:, 5] = torch.cos(theta[:, 2]) * torch.cos(
                theta[:, 0]
            ) + torch.sin(theta[:, 0]) * torch.sin(theta[:, 1]) * torch.sin(theta[:, 2])
            theta_new[:, 6] = torch.cos(theta[:, 2]) * torch.sin(
                theta[:, 1] * torch.sin(theta[:, 0])
            ) - torch.sin(theta[:, 2]) * torch.cos(theta[:, 0])
            theta_new[:, 7] = theta[:, 4]
            ...
            grid_warped = torch.einsum(
                "qijk,bpq->bpijk", grid_padded, theta_new.reshape(-1, 3, 4)
            )
        else:
    ...
    def forward(self, x: List[torch.Tensor], image_size: List[int]) -> torch.Tensor:
        ...
        out = self.rigid_transform(theta) - self.grid
        return out, theta


class GlobalNetRigid(RegUNet):
    """
...
class AdditiveUpSampleBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        resized = torch.sum(
            torch.stack(resized.split(split_size=resized.shape[1] // 2, dim=1), dim=-1),
            dim=-1,
        )
